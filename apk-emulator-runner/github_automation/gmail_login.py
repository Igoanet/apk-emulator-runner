...
def tap_text_contains(xml, sub, desc=""):
    """Clickable node jiska text substring contain karta ho — tap karo.
    2FA chooser ke options ke exact labels vary karte hain, isliye substring match."""
    sl = sub.lower()
    for m in re.finditer(r"<node\b[^>]*>", xml):
        tag = m.group(0)
        tm = re.search(r'text="([^"]*)"', tag)
        if not tm or sl not in tm.group(1).lower():
            continue
        ...
        tap((x1 + x2) // 2, (y1 + y2) // 2, desc or sub)
        return True
    return False
...
def main():
    ...
    while time.time() < deadline:
        ...
        # 2FA method chooser: "Tap Yes on your phone" (Google prompt) option
        # dikhte hi turant select karo — number challenge isi ke baad aata hai
        if tap_text_contains(xml, "tap yes on", "2FA method: Tap Yes on phone") or \
           tap_text_contains(xml, "prompt on your phone", "2FA method: prompt on phone"):
            print("[*] 2FA method choose kiya: Google prompt (Tap Yes)")
            time.sleep(4)
            continue

        # Authenticator/security-code entry screen aaye to humare paas OTP nahi hai —
        # "Try another way" kholke prompt method pe jao
        if "security code" in low or "authenticator" in low:
            if tap_text(xml, "Try another way", "2FA: Try another way"):
                print("[*] Security-code screen — 'Try another way' tap kiya")
                time.sleep(4)
                continue
