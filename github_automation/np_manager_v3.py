...
def handle_tool_result():
    ...
    for attempt in range(40):  # up to ~80s total (mix of 2s and 4s sleeps)
        ...
        # 4d. DEX2C / DEX2C PRO config — engine or license selection screen
        if any("dex2c" in t.lower() or "vm engine" in t.lower() or
               "license" in t.lower() for t in texts) and not submitted:
            print("[DEX2C] DEX2C config screen — selecting engine + confirming")
            # Try to tap "LLVM Engine" or "Default Engine" or just CONFIRM
            for engine_kw in ["LLVM Engine", "Default Engine", "Default", "LLVM"]:
                if engine_kw in texts:
                    tap_text(xml, engine_kw, f"DEX2C engine: {engine_kw}")
                    time.sleep(1)
                    break
            tap_text(get_xml(), "CONFIRM", "DEX2C CONFIRM")
            submitted = True
            time.sleep(8)
            continue

        # 4e. VIEW SIGNATURE / TEST SIGNATURE STRENGTH — just a display screen, dismiss
        if any(kw in xml for kw in ["VIEW SIGNATURE", "Signature Info", "SHA-1", "SHA-256",
                                     "Signature strength", "signature"]) and not submitted:
            for dismiss_kw in ["CLOSE", "Close", "OK", "确定", "BACK", "返回"]:
                if dismiss_kw in texts:
                    tap_text(xml, dismiss_kw, f"Signature view dismiss: {dismiss_kw}")
                    submitted = True
                    time.sleep(2)
                    break
            if submitted:
        ...
        # 4f. ADD CRASH LOG — text input for crash log content, type and confirm
        if any("crash log" in t.lower() or "log content" in t.lower() for t in texts) and not submitted:
            print("[CRASH_LOG] Crash log config — typing content + CONFIRM")
            for txt, x, y in find_any_bounds(xml):
                if "crash" in txt.lower() or "content" in txt.lower() or "input" in txt.lower():
                    adb(f"shell input tap {x} {y}")
                    time.sleep(0.5)
                    adb("shell input text 'Android.CrashReporter.v2.exception.handler'")
                    time.sleep(0.5)
                    break
            tap_text(get_xml(), "CONFIRM", "CrashLog CONFIRM")
            submitted = True
            time.sleep(4)
            continue

        # 4g. XML TRANSLATION SCHEMA — language picker, select English or tap confirm
        if any("translation" in t.lower() or "language" in t.lower() or
               "schema" in t.lower() for t in texts) and not submitted:
            print("[XML_TRANS] XML Translation config — selecting English/CONFIRM")
            for lang_kw in ["English", "EN", "en_US"]:
                if lang_kw in texts:
                    tap_text(xml, lang_kw, f"XML Trans lang: {lang_kw}")
                    time.sleep(1)
                    break
            tap_text(get_xml(), "CONFIRM", "XML_TRANS CONFIRM")
            submitted = True
            time.sleep(5)
            continue

        # 4h. GENERAL EDITOR / DECOMPILE XML — opens an editor, just escape it
        if any("general editor" in t.lower() or "edit xml" in t.lower() or
               "code editor" in t.lower() for t in texts) and not submitted:
            print("[EDITOR] Editor screen detected — escaping")
            adb("shell input keyevent KEYCODE_BACK")
            time.sleep(2)
            submitted = True
            continue

        # 4i. SET ENTRY BY OLD APK — file picker, navigate to input APK
        if any("entry" in t.lower() and "old" in t.lower() for t in texts) and not submitted:
            print("[SET_ENTRY] Set entry config — trying CONFIRM")
            tap_text(xml, "CONFIRM", "SET_ENTRY CONFIRM")
            submitted = True
            time.sleep(4)
            continue

        # 4j. ONE-CLICK APP PROTECTION config — just confirm defaults
        if any("app protection" in t.lower() or "one-click protect" in t.lower()
               for t in texts) and not submitted:
            print("[APP_PROT] One-click app protection config — CONFIRM defaults")
            tap_text(xml, "CONFIRM", "APP_PROT CONFIRM")
            submitted = True
            time.sleep(8)
            continue

        # 4k. DEX SPLIT config — confirm default split count
        if any("dex split" in t.lower() or "split count" in t.lower() or
               "split num" in t.lower() for t in texts) and not submitted:
            print("[DEX_SPLIT] DEX SPLIT config — confirming defaults")
            tap_text(xml, "CONFIRM", "DEX_SPLIT CONFIRM")
            submitted = True
            time.sleep(5)
            continue

        # 4l. APK CLONER — package name input for clone
        if any("clone" in t.lower() and ("package" in t.lower() or "apk" in t.lower())
               for t in texts) and not submitted:
            print("[APK_CLONE] APK Cloner config — using default + CONFIRM")
            tap_text(xml, "CONFIRM", "APK_CLONE CONFIRM")
            submitted = True
            time.sleep(6)
            continue
...
def run_tools():
    """Tap all 39 NP Manager tools from the FUNCTION tools list.
    Ordered by FUD impact — most critical tools first so even partial runs help."""
    TOOLS_TO_RUN = [
        # ── TIER 1: Core obfuscation (highest FUD impact) ─────────────────
        "SUPER OBFUSCATION",
        "CONTROL FLOW OBFUSCATION",
        "APK VM PROTECTION",
        "APK DEX2C PRO",
        ...
        "DEX STRING DECRYPTION",
        "RES CONFUSION 3.0",
        # ── TIER 2: DEX manipulation ──────────────────────────────────────
        "AGAINST DEX CONFUSION",
        "DEX OBFUSCATION DICTIONARY EXTRACTION",
        "OBFUSCATE APK",
        "APK ENCRYPT STRINGS",
        "REMOVE DEBUGGING INFORMATION",
        "RANDOM SOURCEFILE NAME",
        # ── TIER 3: Resource & asset protection ───────────────────────────
        "RES ANTI-RESOURCE OBFUSCATION",
        "ENCRYPT RESOURCE FILES",
        "ENCRYPT ASSET FILE CONTENTS",
        "APK PSEUDO-ENCRYPTION",
        "ANTI-APK PSEUDO-ENCRYPTION",
        # ── TIER 4: Identity & signing ────────────────────────────────────
        "CHANGE PACKAGE NAME OR CLASS NAME",
        "ONE-CLICK RANDOMLY SIGN APK",
        "SIGN APK",
        "INJECTION SIGNATURE VERIFICATION",
        "TEST THE STRENGTH OF THE SIGNATURE CHECK",
        # ── TIER 5: Injection & hardening ─────────────────────────────────
        "ONE-CLICK INJECTION FUNCTION",
        "ONE-CLICK APP PROTECTION",
        "DATA REUSE OPTIMIZATION",
        "APK ALIGNMENT OPTIMIZATION",
        "MODIFY THE FILE TIME IN THE APK",
        # ── TIER 6: DEX structure operations ──────────────────────────────
        "DEX SPLIT",
        "DEX MERGE",
        "ADD CRASH LOG",
        # ── TIER 7: Misc / extras ─────────────────────────────────────────
        "APK CLONER",
        "VIEW SIGNATURE",
        "SET ENTRY BY OLD APK",
        "XML TRANSLATION SCHEMA",
        "DECOMPILE THE XML UNDER RES",
        "THE APK FUNCTION EXTRACTS THE SHELL",
        "XML TRANSLATION SCHEMA",        # second pass (listed twice in strategy)
    ]
